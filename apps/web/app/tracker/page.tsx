        ...
        {editing ?
          <form className='space-y-3'
            onSubmit={async (e) => {
              ...
              const selectedStage = String(form.get('stage') ?? editing.stage ?? 'Interested')
              const rawUrl = String(form.get('url_snapshot') ?? '').trim()
              const normalizedUrl = rawUrl
                ? (/^https?:\/\//i.test(rawUrl) ? rawUrl : `https://${rawUrl}`)
                : null

              try {
                await api.trackerPatch(
                  editing.id,
                  {
                    title_snapshot: form.get('title_snapshot'),
                    org_snapshot: form.get('org_snapshot'),
                    url_snapshot: normalizedUrl,
                    notes: form.get('notes'),
                    deadline: form.get('deadline') || null
                  }
                )

                if (selectedStage !== editing.stage) {
                  await api.trackerPatchStage(editing.id, selectedStage)
                }

                await load()
                setEditing(null)
                toast('Tracker item updated')
              } catch (error: any) {
                toast(error?.message || 'Failed to update tracker item')
              }
            }}
          >
            ...
            <div className='space-y-1'>
              ...
              <Input id='url_snapshot' name='url_snapshot' type='text' inputMode='url' autoComplete='url' defaultValue={editing.url_snapshot || ''} placeholder='https://company.com/jobs/123' />
            </div>
            <div className='space-y-1'>
